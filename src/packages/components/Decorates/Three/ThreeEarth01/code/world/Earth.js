import { Color, DoubleSide, Group, Mesh, MeshBasicMaterial, NormalBlending, Points, PointsMaterial, ShaderMaterial, SphereGeometry, Sprite, SpriteMaterial } from 'three';
import { createAnimateLine, createLightPillar, createPointMesh, createWaveMesh, getCirclePoints } from '../Utils/common';
...
export default class earth {
    ...
    createAnimateCircle() {
         ...
         * 旋转的球
         */
        const ball = new Mesh(new SphereGeometry(this.options.satellite.size, 32, 32), new MeshBasicMaterial({
            color: '#e0b187' // 745F4D
        }));
        const ball2 = new Mesh(new SphereGeometry(this.options.satellite.size, 32, 32), new MeshBasicMaterial({
            color: '#628fbb' // 324A62
        }));
        const ball3 = new Mesh(new SphereGeometry(this.options.satellite.size, 32, 32), new MeshBasicMaterial({
            color: '#806bdf' //6D5AC4
        }));
    ...
    createFlyLine() {
        ...
        this.options.data.forEach(cities => {
            cities.endArray.forEach(item => {
                ...
                const arcline = flyArc(this.options.earth.radius, cities.startArray.E, cities.startArray.N, item.E, item.N, this.options.flyLine);
